...
void loop() {
  ...
  if ( UART.getVescValues() && deltaT > 0) 
  {
    ...
    { 
      ...
      {
        velocity = speed(UART.data.rpm, number_poles, wheel_diameter);
        ...
        double new_rpm_value = new_rpm(new_velocity, number_poles, wheel_diameter);
        if (verbose)
        { 
